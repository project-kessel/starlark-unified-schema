...
workspace = resource("features", extends=rbac_workspace, fields={
    ...
    "excluded_inherited_desired_services": wildcard(service),
    "excluded_inherited_paid_services": wildcard(service)
}, permissions={
    "_paid_services": lambda w: any(
        w.direct_billing_account.services,
        w.parent._paid_services.exclude(w.excluded_inherited_paid_services)
    ),
    "_desired_services": lambda w: any(
        w.direct_service_preferences,
        w.desire_all_services,
        w.parent._desired_services.exclude(w.excluded_inherited_desired_services)
    ),
